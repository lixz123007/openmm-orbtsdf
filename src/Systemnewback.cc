        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //重建
    voxel_grid_origin_x = -0.25f; // Location of voxel grid origin in base frame camera coordinates
    voxel_grid_origin_y = -0.25f;
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const int times)
{
    ...
    // cv::Mat depthmapmid=depthmap;
    // if(times!=1){depthmapmid=depthmaplast;}

    // depthmaplast=depthmap;
    ...
    //checkCUDA(__LINE__, cudaGetLastError());
    float cam2base[]={Tcw.at<float>(0,0),Tcw.at<float>(1,0),Tcw.at<float>(2,0),Tcw.at<float>(0,3),
    Tcw.at<float>(0,1),Tcw.at<float>(1,1),Tcw.at<float>(2,1),Tcw.at<float>(1,3),
    Tcw.at<float>(0,2),Tcw.at<float>(1,2),Tcw.at<float>(2,2),Tcw.at<float>(2,3),
    ...
    //checkCUDA(__LINE__, cudaGetLastError());
    //demo();
     cout<<"INput: cam_K "<<gpu_cam_K<<"cam2base"<<gpu_cam2base<<"depth_im"<<gpu_depth_im<<"im_height"<<depthmap.rows<<"im_width:"<<depthmap.cols<<endl;
     cout<<"voxel_grid_dim_x "<<voxel_grid_dim_x<<"voxel_grid_origin_x"<<voxel_grid_origin_x<<"voxel_size"<<voxel_size<<"gpu_voxel_grid_TSDF"<<gpu_voxel_grid_TSDF<<endl;
