...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp,const int times)
{
    ...
    cv::imwrite("/home/finch/catkin_ws/src/ORB_SLAM2/b1.jpg", im);
    ...
    // 添加python文件所在的位置
    PyRun_SimpleString("sys.path.append('/home/finch/test')");
    ...
    //cvtColor(im, output_image, CV_BGR2RGB);
    black=cv::imread("/home/finch/catkin_ws/src/ORB_SLAM2/black.jpg", 0);
    ...
    for (int c = 0; c <depthmap.cols ; ++c) {
     if(black.ptr<uchar>(r)[c]!=0){continue;}//||black.at<Vec3b>(r, c)[1]!=0||black.at<Vec3b>(r, c)[2]!=0)continue;//mask way to do!cout<<r<<"r c: "<<c<<" result:"<<black.ptr<uchar>(r)[c]<<endl;
